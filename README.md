...
Infrastructure and network diagram generated by [nix-topology](https://github.com/oddlama/nix-topology) directly from the NixOS configs.

```bash
# Build the diagram (outputs SVGs into ./result/)
nix build .#topology
...
The output directory contains one SVG per logical view (e.g. `main.svg`, `network.svg`). Open them in any browser or SVG viewer.

To rebuild after config changes, re-run `nix build .#topology`.

> **Note:** The diagrams are generated from `topology.*` options auto-extracted from each NixOS host. To add custom annotations (services, links, colours) create `topology.nix` and add it to `perSystem.topology.modules` in `flake.nix`.
