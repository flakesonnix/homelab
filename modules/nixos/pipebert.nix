{
  ...
  reloadUsbScript = pkgs.writeShellApplication {
    name = "pipebert-reload-usb";
    runtimeInputs = [pkgs.coreutils pkgs.gawk pkgs.uhubctl pkgs.usbutils];
    text = ''
      if [[ $# != 2 ]]; then
        ...
        user.services = {
          ...
          pipewire.serviceConfig =
            ...
            // lib.optionalAttrs cfg.usbDevice.enableReload {
              ExecStartPost = "-${reloadUsbScript}/bin/pipebert-reload-usb ${lib.escapeShellArg cfg.usbDevice.vendorId} ${lib.escapeShellArg cfg.usbDevice.productId}";
            };
        };
      }
    ];
