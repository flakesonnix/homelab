...
pub fn generateFiltered(program: *const ast.Program, allocator: std.mem.Allocator, filter_host: ?[]const u8) ![]const u8 {
    ...
    for (program.decls) |decl| if (decl == .host) {
        if (filter_host) |fh| {
            if (!std.mem.eql(u8, decl.host.name.name, fh)) continue;
        }
        try hosts.append(allocator, decl.host);
    };
