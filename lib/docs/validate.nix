...
{lib}: let
  ...
  validateEntity = entity: let
    errors =
      lib.optional (entity.id == "" || entity.id == null) "missing id"
  ...
  in
    entityErrors ++ dupErrors;

  validateRelations = entities: let
    ...
    checkTargets = lib.concatMap (
      ...
      in
        lib.concatMap (
          t: let
            exists = elem t allIds || elem t (map (k: let parts = lib.splitString ":" k; in "${lib.head parts}:${lib.last parts}") allIds);
          ...
          in
            lib.optional (!existsId) "${id} references unknown ${t}"
        )
        targets
    ) (attrNames entities);
  in
