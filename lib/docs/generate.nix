...
{lib, pkgs}: let
  ...
  loadSource = {hosts, roles, bundles, presets, microvms, modules}:
    let
      # Hosts from data/hosts + hosts/mireo microVMs
      hostEntities = lib.mapAttrs (name: cfg:
        domain.mkHost {
          id = name;
          title = name;
      ...
      roleEntities = lib.mapAttrs (name: r:
        domain.mkRole {
          id = name;
          title = name;
      ...
      bundleEntities = lib.mapAttrs (name: b:
        domain.mkBundle {
          id = name;
          title = name;
      ...
      presetEntities = lib.mapAttrs (name: p:
        domain.mkPreset {
          id = name;
          title = name;
      ...
      microvmEntities = lib.mapAttrs (name: v:
        domain.mkMicroVM {
          id = name;
          title = name;
      ...
      moduleEntities = lib.mapAttrs (name: m:
        domain.mkModule {
          id = name;
          title = name;
      ...
      blocks = [
        ...
        (ast.mkSection {
          ...
          blocks = [
            (ast.mkList {
              ordered = false;
              items = map (s: ast.mkReference {type = "service"; id = s;}) host.services;
            })
          ];
        })
        (ast.mkSection {
          ...
          blocks = [
            (ast.mkList {
              ordered = false;
              items = map (v: ast.mkReference {type = "microvm"; id = v;}) host.microvms;
            })
          ];
        })
      ];
    ...
    let
      ...
      indexDoc = ast.mkDocument {
        ...
        blocks = [
          (ast.mkList {
            ordered = false;
            items = map (name: ast.mkLink {text = name; url = "hosts/${name}.md";}) (lib.attrNames (lib.filterAttrs (n: v: v.type == "host") entities));
          })
        ];
      };
