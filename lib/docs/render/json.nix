...
{lib}: let
  ...
  renderEntity = entity: {
    ...
    // For Host, include network, services, etc.
    // We just pass through the whole entity except _type.
    // To keep it deterministic, we remove _type and sort.
    ;
  ...
  renderGraph = entities:
    let
      nodes = map (e: { id = "${e.type}:${e.id}"; type = e.type; label = e.title; }) (lib.attrValues entities);
      edges = lib.concatLists (map (e:
        let rels = e.relations or {};
        in lib.concatLists (lib.mapAttrsToList (rel: targets:
          map (t: { from = "${e.type}:${e.id}"; to = t; label = rel; }) targets
        ) rels)
